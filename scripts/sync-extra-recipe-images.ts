...
const source = fs.readFileSync("src/data/extra-recipes.ts", "utf8");

const blocks = source.split(/\n\s*},\s*\n/g);
const entries: Array<{ title: string; originalTitle: string; image: string }> = [];

for (const block of blocks) {
  const title = block.match(/title:\s*["']([^"']+)["']/)?.[1];
  const originalTitle = block.match(/original_title:\s*["']([^"']+)["']/)?.[1];
  const image = block.match(/image:\s*["']([^"']+)["']/)?.[1];

  if (title && originalTitle && image) {
    entries.push({ title, originalTitle, image });
  }
}
